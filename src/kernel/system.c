...
_PROTOTYPE( struct proc* get_process_from_pid, (pid_t) );
FORWARD _PROTOTYPE( int do_chgroup, (message *m_ptr) );
...
/* Main entry point of sys_task.  Get the message and dispatch on type. */
  ...
  while (TRUE) {
	...
	switch (m.m_type) {	/* which system call */
      ...
      case SYS_CHGROUP: r = do_chgroup(&m); break;
...
struct proc * get_process_from_pid(int pid)
{
	int I;
...
PRIVATE int do_chgroup(m_ptr)
message *m_ptr;
{
  struct proc* rp;

  if((rp = get_process_from_pid(m_ptr->m1_i1)) != NIL_PROC){
    if(is_proc_ready(rp)){
      lock_unready(rp);
      rp->group = 1 + !(rp->group - 1);
      lock_ready(rp);
      return 2;
    }
    rp->group = 1 + !(rp->group - 1);
