...
PRIVATE int sched_gticks[3] = {0, SCHED_RATE, 2*SCHED_RATE};
 ...
 */
  ...
  if (--sched_ticks == 0) {
	if (bill_ptr == prev_ptr) lock_sched(bill_ptr->group);	/* process has run too long */
 ...
 */
  ...
  if (next_alarm <= now ||
      sched_ticks == 1 &&
      bill_ptr == prev_ptr &&
      rdy_head[USER_QA + bill_ptr->group-1] != NIL_PROC) {
	interrupt(CLOCK);
  ...
  if (--sched_ticks == 0) {
	...
	sched_ticks = sched_gticks[bill_ptr->group];	/* reset quantum */
	prev_ptr = bill_ptr;		/* new previous process */
  }
