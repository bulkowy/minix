...
FORWARD _PROTOTYPE( void sched, (int group) );
...
int sched_max_gsizes[3] = {0, 5, 3};
int sched_gsizes[3] = {0, 0, 0};
int default_sched_group = 1;

PUBLIC int is_proc_ready(rp)
struct proc * rp;
{
	struct proc * xp;
	xp = rdy_head[USER_QA + rp->group - 1];
	while(xp != NIL_PROC)
	{
		if(xp == rp)
			return 1;
 ...
 */
  ...
  if ( (rp = rdy_head[USER_QA]) != NIL_PROC ||
      rdy_head[USER_QB] != NIL_PROC) {
    if(!proc_ptr){
	    proc_ptr = rp;
      bill_ptr = rp;
      return;
    }
    prev_group = proc_ptr->group;
    next_queue = USER_QA + ( !(prev_group-1));
    if ( (rp = rdy_head[next_queue]) != NIL_PROC) {
      proc_ptr = rp;
      bill_ptr = rp;
      return;
    }
    else{
      prev_queue = USER_QB - ( !(prev_group-1));
      proc_ptr = rdy_head[prev_queue];
      bill_ptr = rdy_head[prev_queue];
      return;
    }
  }
  if ( (rp = rdy_head[USER_QB]) != NIL_PROC) {
    proc_ptr = rp;
    bill_ptr = rp;
    return;
  }
 ...
 *   USER_QA  - (lowest priority) Group A of USER tasks
 *   USER_QB  - (lowest priority) Group B of USER tasks 
 */
  int GROUP_Q;
  ...
  if (istaskp(rp)) {
	if (rdy_head[TASK_Q] != NIL_PROC)
		...
		proc_ptr = rp;	/* run fresh task next */
   ...
   * bound processes.)
   */
  GROUP_Q = USER_QA + rp->group - 1;
  if (rdy_head[GROUP_Q] == NIL_PROC){
  	rdy_head[GROUP_Q] = rp;
  }
  else{
    rdy_tail[GROUP_Q]->p_nextready = rp;
  }
  rdy_tail[GROUP_Q] = rp;
  rp->p_nextready=0;
  if (sched_gsizes[rp->group] >= sched_max_gsizes[rp->group])
  	unready(rdy_head[GROUP_Q]);
  
  sched_gsizes[rp->group]++;
  return;
}
...
/* A process has blocked. */
  ...
  if (istaskp(rp)) {
	...
	qtail = &rdy_tail[SERVER_Q];
  	...
  	GROUP_Q = USER_QA +rp->group-1;
  	if(rp == rdy_head[GROUP_Q])
  		rdy_head[GROUP_Q] = rp->p_nextready;
  	if(rp == proc_ptr)
  		pick_proc();
    sched_gsizes[rp->group]--;
  	return;
 ...
 */
  ...
  int GROUP_Q = USER_QA + group - 1;  
  if (rdy_head[GROUP_Q] == NIL_PROC) return;

  /* One or more user processes queued. */
  rdy_tail[GROUP_Q]->p_nextready = rdy_head[GROUP_Q];
  rdy_tail[GROUP_Q] = rdy_head[GROUP_Q];
  rdy_head[GROUP_Q] = rdy_head[GROUP_Q]->p_nextready;
  rdy_tail[GROUP_Q]->p_nextready = NIL_PROC;
  pick_proc();
}
 ...
 *				lock_sched				    *
 *==========================================================================*/
PUBLIC void lock_sched(int group)
{
